...
- [ ] Install Debian 12 on Node 1 (172.16.91.101)
- [ ] Install Debian 12 on Node 2 (172.16.91.102)
...
- [ ] Verify VIP is on Node 1
  ```bash
  ip addr show | grep 172.16.91.100
  ```
...
- [ ] Both nodes: Deploy kamailio.cfg
  ...
  # Edit: Set IP addresses, DB connection to VIP 172.16.91.100
  ```
- [ ] Both nodes: Configure db_mode=2 (write-back)
  ```cfg
  # In kamailio.cfg:
  modparam("usrloc", "db_mode", 2)
  modparam("usrloc", "db_url", "postgres://kamailio:password@172.16.91.100/voip_platform")
  ```
- [ ] Node 1: Insert FreeSWITCH into dispatcher
  ```sql
  INSERT INTO kamailio.dispatcher (setid, destination, flags, description)
  VALUES (1, 'sip:172.16.91.101:5080', 0, 'FreeSWITCH Node 1'),
         (1, 'sip:172.16.91.102:5080', 0, 'FreeSWITCH Node 2');
  ```
...
### SIP Testing
- [ ] Register SIP phone to VIP (172.16.91.100:5060)
- [ ] Check registration
  ```bash
  kamcmd ul.dump
  ```
...
- [ ] Both nodes: Deploy json_cdr.conf.xml
  ...
  # Edit: url=http://172.16.91.100:8080/fs/cdr (VoIP Admin)
  ```
...
- [ ] Create PostgreSQL backup script
  ```bash
  #!/bin/bash
  pg_basebackup -h 172.16.91.100 -U postgres -D /backup/pg-$(date +%Y%m%d) -P
  ```
- [ ] Setup WAL archiving
...
- [ ] Create SIPp scenarios
  ```bash
  sipp -sf scenarios/call.xml -s 1001 172.16.91.100 -r 10 -l 100
  ```
- [ ] Test 100 CC
- [ ] Test 400 CC
- [ ] Test 600 CC
...
- [ ] Backup old system
- [ ] Update DNS/routing to VIP 172.16.91.100
